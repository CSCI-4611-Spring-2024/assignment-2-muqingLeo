...
// unit vector when bouncing from ground plane is (0,1,0)
...
export class App extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void 
    {
        ...
        // ADD YOUR CODE
        if (this.hole.position.x >= this.playArea.max.x - this.holeRadius) {
            this.hole.position.x = this.playArea.max.x - this.holeRadius;
        }
        else if (this.hole.position.x <= this.playArea.min.x + this.holeRadius) {
            this.hole.position.x = this.playArea.min.x + this.holeRadius;
        }
        if (this.hole.position.z >= this.playArea.max.z - this.holeRadius) {
            this.hole.position.z = this.playArea.max.z - this.holeRadius;
        }
        else if (this.hole.position.z <= this.playArea.min.z + this.holeRadius) {
    ...
    handleBoundaryCollision(rb: RigidBody, frictionSlowDown: number): void
    {
        ...
        // collision handler for y axis
        if (rb.position.y - rb.getRadius() <= rb.getRadius()) {
            rb.position.y = rb.getRadius();
            rb.velocity.y *= -1;
            rb.velocity.y *= 0.85;
        }
        // collision handler for x axis
        if (rb.position.x - rb.getRadius() <= this.playArea.min.x) {
            rb.position.x = this.playArea.min.x + rb.getRadius();
            rb.velocity.reflect(new gfx.Vector3(1, 0, 0));
    ...
    handleObjectCollision(rb1: RigidBody, rb2: RigidBody, frictionSlowDown: number): void
    {
        ...
        const normal_vector = gfx.Vector3.subtract(rb1.position, rb2.position);

        const penetration = (rb1.getRadius() + rb2.getRadius()) - rb1.position.distanceTo(rb2.position);
        ...
        if (collision_detected == true) {
            normal_vector.normalize(); // unit vector correction
            const collision_correction = gfx.Vector3.multiplyScalar(normal_vector, penetration / 2);
            rb1.position.add(collision_correction);
            rb2.position.subtract(collision_correction);

            rb1.velocity.reflect(normal_vector);
            rb2.velocity.reflect(normal_vector);
